"""
title: Victor DCS - CodeGemma
author: VictorRomeoSierra
version: 1.0.0
license: MIT
description: Victor DCS Assistant using CodeGemma model
...
class Pipe:
    ...
    def __init__(self):
        self.type = "pipe"
        self.id = "victor_codegemma"
        self.name = "Victor DCS - CodeGemma"
        self.valves = self.Valves()
        
        # Model configuration
        self.ollama_model = "codegemma:latest"
    ...
    def pipe(self, body: dict) -> str:
        """Routes requests through N8N webhook for DCS-enhanced responses using CodeGemma."""
        ...
        try:
            ...
            if self.valves.debug:
                print(f"[Victor-CodeGemma] Processing: {user_message[:50]}...")
                print(f"[Victor-CodeGemma] Timeout: {self.valves.timeout}s, Model: {self.ollama_model}")
            ...
            if self.valves.debug:
                print(f"[Victor-CodeGemma] Sending request to N8N...")
            ...
            if self.valves.debug:
                print(f"[Victor-CodeGemma] Received response: {response.status_code}")
