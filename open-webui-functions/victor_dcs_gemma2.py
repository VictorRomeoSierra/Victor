"""
title: Victor DCS - Gemma 2
author: VictorRomeoSierra
version: 1.0.0
license: MIT
description: Victor DCS Assistant using Gemma 2 model
...
class Pipe:
    ...
    def __init__(self):
        self.type = "pipe"
        self.id = "victor_gemma2"
        self.name = "Victor DCS - Gemma 2"
        self.valves = self.Valves()
        
        # Model configuration
        self.ollama_model = "gemma2:latest"
    ...
    def pipe(self, body: dict) -> str:
        """Routes requests through N8N webhook for DCS-enhanced responses using Gemma 2."""
        ...
        try:
            ...
            if self.valves.debug:
                print(f"[Victor-Gemma 2] Processing: {user_message[:50]}...")
                print(f"[Victor-Gemma 2] Timeout: {self.valves.timeout}s, Model: {self.ollama_model}")
            ...
            if self.valves.debug:
                print(f"[Victor-Gemma 2] Sending request to N8N...")
            ...
            if self.valves.debug:
                print(f"[Victor-Gemma 2] Received response: {response.status_code}")
