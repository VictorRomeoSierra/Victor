"""
title: Victor DCS - DeepSeek Coder v2
author: VictorRomeoSierra
version: 1.0.0
license: MIT
description: Victor DCS Assistant using DeepSeek Coder v2 model
...
class Pipe:
    ...
    def __init__(self):
        self.type = "pipe"
        self.id = "victor_deepseek_coder_v2"
        self.name = "Victor DCS - DeepSeek Coder v2"
        self.valves = self.Valves()
        
        # Model configuration
        self.ollama_model = "deepseek-coder-v2:latest"
    ...
    def pipe(self, body: dict) -> str:
        """Routes requests through N8N webhook for DCS-enhanced responses using DeepSeek Coder v2."""
        ...
        try:
            ...
            if self.valves.debug:
                print(f"[Victor-DeepSeek Coder v2] Processing: {user_message[:50]}...")
                print(f"[Victor-DeepSeek Coder v2] Timeout: {self.valves.timeout}s, Model: {self.ollama_model}")
            ...
            if self.valves.debug:
                print(f"[Victor-DeepSeek Coder v2] Sending request to N8N...")
            ...
            if self.valves.debug:
                print(f"[Victor-DeepSeek Coder v2] Received response: {response.status_code}")
